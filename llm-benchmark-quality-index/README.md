...
# Compute inter-rater reliability across the three evaluators
python 03_iaa/compute_iaa.py

# Compute BQAI scores and tier assignments
...
Reproduces the results from the paper using the three-evaluator real scoring data
in `02_scoring/raw_scores/`. AHP consistency ratio CR = 0.023; mean ICC(2,k) =
0.855 (good agreement); HLE is the sole Tier A benchmark (BQAI 0.831), with 10
Tier B and 19 Tier C benchmarks.

### Re-run scoring with new evaluators

```bash
# After placing filled Excel scoring sheets in 02_scoring/raw_scores/
...
Applying the BQAI to the 30-benchmark sample (48% of the curated 63-benchmark
corpus) under a **three-evaluator blinded protocol** yields a notably conservative
tier distribution:

- **Tier A (BQAI ≥ 0.82):** **HLE** (0.831) is the sole Tier A benchmark.
- **Tier B (0.70–0.82):** 10 benchmarks—**LiveBench** (0.786), **LiveCodeBench**
  (0.772), **HELM** (0.769), **HarmBench** (0.727), **SWE-bench Verified**
  (0.720), CLadder (0.719), OlympiadBench (0.716), MMMU (0.714), MMLU-Pro
  (0.710), BBQ (0.707).
- **Tier C (< 0.70):** 19 benchmarks, including foundational instruments that
  fall below the moderate-quality threshold due to saturation and contamination
  vulnerabilities: HumanEval (0.634), MMLU (0.589), GSM8K (0.587), HellaSwag
  (0.583), HHH (0.571), FrontierMath (0.558).

**Inter-rater reliability** is strong across dimensions: mean ICC(2,k) = 0.855
(range 0.744–0.943, *good* agreement); mean quadratic-weighted Cohen's κ = 0.541
(*moderate* agreement). The largest evaluator disagreement is in Q5
(robustness/contamination, mean |Δ| = 0.162), reflecting genuine conceptual
variation about how to penalize benchmark saturation. Full IAA report:
[`03_iaa/iaa_report.md`](03_iaa/iaa_report.md).

**Sensitivity:** Tier classifications remain robust across Monte Carlo
perturbation (29/30 stable at ±10%) and four realistic alternative weighting
schemes (Kendall τ ≥ 0.82, tier agreement ≥ 80%). Full sensitivity report:
[`04_bqai/sensitivity_report.md`](04_bqai/sensitivity_report.md).

The conservative outcome under multi-evaluator absolute-agreement scoring is a
substantive finding: very few benchmarks satisfy all seven quality dimensions
simultaneously when applied with rigor across independent evaluators. This
discriminative power is precisely what the BQAI is designed to provide.
